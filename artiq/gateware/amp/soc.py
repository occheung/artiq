...
class AMPSoC:
    ...
    def __init__(self):
        ...
        self.add_wb_slave(self.mem_map["mailbox"], 8*mailbox_size,
                          self.mailbox.i1)
        self.kernel_cpu.add_wb_slave(self.mem_map["mailbox"], 8*mailbox_size,
                                     self.mailbox.i2)
    ...
    def register_kernel_cpu_csrdevice(self, name, csrs=None):
        print("Adding kernel CSR device:", name)
        if csrs is None:
            csrs = getattr(self, name).get_csrs()
        bank = wishbone.CSRBank(csrs)
        self.submodules += bank
        self.kernel_cpu.add_wb_slave(self.mem_map[name], 8*2**bank.decode_bits, bank.bus)
